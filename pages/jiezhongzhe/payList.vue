	...
	<view>
		<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(value, key) in list" :key="key" style="margin-top: 10rpx;">
			<view class="uni-media-list" style="border-bottom: 2rpx solid #eeeeee;">
				<view>
					<text style="line-height: 100rpx; font-size: 30rpx;">疫苗名称：{{value.va_name}}</text>
				</view>
				<view class="uni-media-list-body">
					...
					<view class="uni-media-list-text-bottom">
						<text style="color: red;">支付金额￥：{{value.cost}}</text>
					</view>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			init(){
				this.$u.get(`/user/findPayHistories?userId=${uni.getStorageSync("userInfo").user.id}`).then(res=>{
					console.log(res)
					if(res.code === 0){
						this.list=res.data;
					}
