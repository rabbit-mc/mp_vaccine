...
<script>
	export default {
		data() {
			return {
				...
				form:{
					...
					part:"左臂",
					note:"",
					workid: uni.getStorageSync("userInfo").worker.id
				}
				// list:[],
				// show:false
				// rules : {
		...
		onLoad(option) {
			this.form.appointId = option.id
			// console.log(uni.getStorageSync("userInfo").worker.id)
		},
		methods: {
			scanCode(){
				this.isShow=false;
			...
			jieZhong(isSucceeded){
				this.isLoading=true;
				if(isSucceeded==1){
					this.$u.post("/worker/inoculate/nosave",this.form).then(res=>{
					...
					this.$u.post("/worker/inoculate/save",this.form).then(res=>{
						if(res.code == 0){
							this.$refs.uToast.show({
								title :res.msg,
								type : 'success'
